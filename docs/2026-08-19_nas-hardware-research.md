...
> **CORRECTED 2026-09-07**: §9 below states the RAM "must come pre-installed (see the F4-425
> Plus)". **Build D's "RAM-comfortable" premise no longer holds**, but the reason is not the one
> first recorded — see the vendor datasheets now in [`download/`](../download/), which supersede
> both the Amazon listing and this section:
>
> - **Every model in this range has ONE SODIMM slot** (`Total Memory Slot Number: 1 (DDR5
